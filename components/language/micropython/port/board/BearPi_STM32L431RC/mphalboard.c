...
STATIC void mp_hal_extint_config(mp_hal_pin_obj_t pin, uint32_t mode) {
    GPIO_InitTypeDef GPIO_InitStruct;
    GPIO_InitStruct.Pin = no2mask(pin->pin_no);
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
...
void mp_hal_extint_register_pin(mp_hal_pin_obj_t pin, uint32_t mode, mp_obj_t handler) {
